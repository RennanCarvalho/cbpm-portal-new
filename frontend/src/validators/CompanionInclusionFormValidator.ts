...
const CompanionInclusionValidator = Yup.object().shape({
  ...
  nomeTestemunha: Yup.string().trim().required('Campo obrigatório'),

  dataNascimentoTestemunha: Yup.string()
    .matches(
      /(0[1-9]|[12][0-9]|3[01])\/(0[1-9]|1[012])\/(19|20)\d{2}/,
      'Formato correto 00/00/0000'
    )
  ...
  cpfTestemunha: Yup.string()
    .min(10, 'O CPF deve conter 11 dígitos')
    .max(11, 'O CPF deve conter 11 dígitos')
    .trim()
    .required('Campo obrigatório'),

  rgTestemunha: Yup.string()
    .min(6, 'O RG deve ter no mínimo 6 dígitos')
    .max(12, 'O RG deve ter no máximo 12 dígitos')
    .trim()
    .required('Campo obrigatório'),

  rgdgTestemunha: Yup.string()
    .max(1, 'Deve ter somente um dígito')
    .trim()
    .required('Campo obrigatório'),

  numeroTestemunha: Yup.string()
    .max(6, 'O número deve ter no máximo 6 dígitos')
    .trim()
    .required('Campo obrigatório'),

  complementoTestemunha: Yup.string().trim().optional(),

  cepTestemunha: Yup.string()
    .min(8, 'O CEP deve ter 8 dígitos')
    .max(8, 'O CEP deve ter 8 dígitos')
    .trim()
    .required('Campo obrigatório'),

  telTestemunha: Yup.string()
    .min(8, 'O telefone deve conter no mínimo 8 dígitos')
    .max(13, 'O telefone deve conter no máximo 13 dígitos')
    .trim()
    .required('Campo obrigatório'),
...
export default CompanionInclusionValidator;
