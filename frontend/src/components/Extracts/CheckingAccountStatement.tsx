...
import { CheckingAccountPDF } from '../../components/PrintPDF/CheckingAccountPDF';
import { setupAPIClient } from '../../services/setupAPIClient';
import InputMask from 'react-input-mask';
import Link from '../Link';
import CheckingAccountStatementTableData from './CheckingAccountStatementTableData';
...
const CheckingAccountStatement = ({ title }: Title) => {
  ...
  async function handleSearch(payload: SearchData): Promise<void> {
    setLoad(false);
    try {
      const response = await api.get<HandleSearch>(
        '/user/checkingAccount-extract',
        {
          params: {
      ...
      const lengthData = response.data.data.length;
      const lastData = response.data.data[lengthData - 1];

      if (lastData !== undefined) {
        //@ts-ignore
        const mes = lastData.MesCompetencia.toString().padStart(2, '0');
        //@ts-ignore
        const ano = lastData.AnoCompetencia.toString();
        const mesAnoCompetencia = `${mes}/${ano}`;
        if (mesAnoCompetencia !== payload.periodoFinal) {
  ...
  async function handleReport(payload: SearchData): Promise<any> {
    setLoad(false);
    try {
      const response = await api.get('/user/checkingAccount-extract/', {
        params: {
          periodoInicial: payload.periodoInicial,
          periodoFinal: payload.periodoFinal,
        },
  ...
  return (
    <VStack>
      ...
      {data ? <CheckingAccountStatementTableData {...data} /> : ''}
      <Text>
        <br />
        <br />
