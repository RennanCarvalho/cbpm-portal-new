...
export async function MedicalExpenseStatementPDF(
  extract: any,
  initialDate: any,
  finalDate: any
) {
  ...
  const columnsTitle: TableCell[] = [
    ...
    { text: 'CBPM', style: 'columnsTitle' },
    { text: 'CONTRIBUINTE', style: 'columnsTitle' },
  ];
  ...
  for await (let data of extract.data) {
    ...
    rows.push({ text: data.NomePaciente, style: 'textBody' });
    ...
    rows.push({ text: `${formatMoney(data.PgCBPM)}`, style: 'textBody' });
    rows.push({
      text: `${formatMoney(data.PgContribuinte)}`,
      style: 'textBody',
    });
  ...
  const extractTitle = 'Detalhamento Mensal das Despesas Médicas - Coparticipação';
  ...
  Processado em ${todayDate}. Rua Alfredo Maia, 218 - Luz - São Paulo - SP - CEP: 01106-010 - Fone: (11) 3315- 3075  Copyright © 2022 - Caixa Beneficiente da Polícia Militar
  `;
  ...
  const docDefinitions: TDocumentDefinitions = {
    ...
    content: [
      ...
      },
      {
        ...
        table: {
          headerRows: 1,
          widths: ['10%', '10%', '20%', '25%', '8%', '10%', '10%', 'auto'],
          body: body,
        },
      },
      {
